...
const ID_REFERENCING_ATTRIBUTES_SET = /*@__PURE__*/new Set(['aria-activedescendant', 'aria-controls', 'aria-describedby', 'aria-details', 'aria-errormessage', 'aria-flowto', 'aria-labelledby', 'aria-owns', 'for', 'popovertarget']);
...
function computeShadowMode(def, owner, renderer, hydrated) {
  ...
  process.env.NODE_ENV === 'test-lwc-integration' && process.env.FORCE_NATIVE_SHADOW_MODE_FOR_TEST ||
 ...
 * @returns {RecordModel}
 */
function normalizeRecord(objectApiName, fields) {
 ...
 */
function assertDataAdapter(adapter) {
  const requiredMethods = ['getRuntime', 'loadRecord', 'queryRecords', 'createRecord', 'updateRecord', 'deleteRecord', 'getObjectSchema', 'getLayout', 'getListViewInfo', 'getListViewMetadata', 'resolveReferenceDisplay', 'getCapabilities'];
 ...
 */
class PulsarDataAdapter extends DataAdapter {
  /**
  ...
  async loadRecord(objectApiName, id) {
    ...
    const rows = await sdk.read(objectApiName, {
      Id: id
    });
  ...
  async getLayout(input) {
    ...
    if (!sdk?.getLayout) {
      console.log('[PulsarDataAdapter] sdk.getLayout unavailable', {
        input
      });
      return {
        sections: []
...
function mapDescribeLayout(describeLayout, mode) {
  const sourceSections = mode === 'edit' ? describeLayout?.editLayoutSections : describeLayout?.detailLayoutSections;
  return {
...
function mapLayoutComponent(component) {
  const details = component?.details && typeof component.details === 'object' ? component.details : null;
  return {
...
class Pulsar {
  ...
  */
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (Array.isArray(relationshipField.referenceTo)) {
        if (relationshipField.referenceTo.length === 1) {
          ...
          refType = currentRecord?.[relationshipField.name + '__r']?.attributes?.type || currentRecord?.[relationshipField.name]?.attributes?.type || relationshipField.referenceTo[0]; // fallback guess
        }
      }
      if (!refType) return null;
      const results = await this.read(refType, {
        Id: refId
      });
  ...
  async syncData(options = {}) {
    const validKeys = ['singleObjectSyncEnabled', 'rootObjectId', 'parentIdFieldList', 'childRelationshipList', 'pushChangesSyncEnabled', 'useComposite', 'useCompositeGraph'];
    const data = {};
  ...
  */
  ...
  async chatterPostFeed(message, parentId, parentFeedItemId) {
    ...
    const data = {
      ...
      Parent: parentId,
      ...(parentFeedItemId && {
        ParentFeedItem: parentFeedItemId
      })
    };
    const response = await this._send({
  ...
  */
  async getPicklist(objectName, fieldName, recordTypeId, controllerFieldName, controllerFieldValue) {
    const data = {
      ...
      ...(controllerFieldName && controllerFieldValue && {
        [controllerFieldName]: controllerFieldValue
      })
    };
  ...
  */
  async mail(to, cc, attach, subject, body) {
    return this._send({
      type: 'mail',
      data: {
        ...(Array.isArray(to) && to.length && {
          to
        }),
        ...(Array.isArray(cc) && cc.length && {
          cc
        }),
        ...(Array.isArray(attach) && attach.length && {
          attach
        }),
 ...
 * @param {string} height
 */
function notifyHostSize(height = '*') {
 ...
 * @returns {() => void}
 */
function installHostSizing(root) {
  notifyHostSize('*');
 ...
 */
function parseLaunchContext(search) {
